...
class AuthRepository(
    ...
    suspend fun verifyWhatsAppLoginOtp(phone: String, otp: String): ProfileDetails {
        ...
        if (!result.accountExists) {
            throw WhatsAppSignupRequiredException(result.normalizedPhone ?: normalizePhone(phone))
        }
    ...
    suspend fun verifyAdminWhatsAppLogin(phone: String, otp: String): ProfileDetails {
        testAdminLoginFor(phone)?.let { login ->
            if (login.otp == otp) return signInAdmin(login.email, login.password)
...
class WhatsAppSignupRequiredException(val phone: String) : Exception(
    "OTP verified. Please create your account to continue."
)
