...
async function findProfileForPhone(phone: string): Promise<ProfileRow | null> {
  const admin = adminClient();
...
async function verifyOtpRequest(req: Request): Promise<Response> {
  ...
  const profile = await findProfileForPhone(phone);
  if (!profile) {
    console.info("WhatsApp OTP verified for new account", { phone });
    return jsonResponse({
      verified: true,
      account_exists: false,
      normalized_phone: phone,
      message: "OTP verified. Please create your account to continue.",
    });
  }
